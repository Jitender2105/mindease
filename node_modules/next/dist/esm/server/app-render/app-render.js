...
async function spawnDynamicValidationInDev(resolveValidation, tree, ctx, isNotFound, clientReferenceManifest, route, requestStore) {
    ...
    try {
        const prerender = require('react-dom/static.edge').prerender;
        const { prelude: unprocessedPrelude } = await prerenderClientWithPhases(()=>workUnitAsyncStorage.run(finalClientPrerenderStore, prerender, /*#__PURE__*/ _jsx(App, {
                ...
                onError: (err, errorInfo)=>{
                    ...
                    if (isPrerenderInterruptedError(err) || finalClientController.signal.aborted) {
                        ...
                        if (typeof componentStack === 'string') {
                            trackAllowedDynamicAccess(route, componentStack, dynamicValidation);
                        }
                        return;
                    }
                    return getDigestForWellKnownError(err);
    ...
    function LogDynamicValidation() {
        try {
            ...
            throwIfDisallowedDynamic(route, preludeIsEmpty, dynamicValidation, serverDynamicTracking, clientDynamicTracking);
        } catch  {}
        return null;
    }
    ...
    try {
         ...
         */ // Prerender controller represents the lifetime of the prerender.
                ...
                if (!ctx.renderOpts.doNotThrowOnEmptyStaticShell) {
                    throwIfDisallowedDynamic(workStore.route, preludeIsEmpty, dynamicValidation, serverDynamicTracking, clientDynamicTracking);
                }
                const getServerInsertedHTML = makeGetServerInsertedHTML({
                    polyfills,
                    renderServerInsertedHTML,
